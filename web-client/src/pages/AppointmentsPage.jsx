...
import { Activity, Plus, Search, Filter, Clock, Video, MapPin, MoreVertical, Calendar as CalendarIcon, Check, X, User } from 'lucide-react';
...
export default function AppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      // Sort: newly added (by createdAt) at the top, cancelled at the bottom
  ...
  const filteredAppointments = appointments.filter(a => {
    ...
    const matchesSpecialization = (a.specialization || '').toLowerCase().includes(searchLower);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-xl font-medium">
            {isDoctor ? 'Manage your patient consultations and daily clinical slots.' : 'View and manage your upcoming medical consultations.'}
          </p>
        </div>
              ...
              <div className="flex flex-col md:flex-row h-full max-h-[90vh]">
                <div className="flex-1 p-12 space-y-10 overflow-y-auto bg-white">
                  <DialogHeader className="space-y-4">
                    <DialogTitle className="text-4xl font-black text-primary tracking-tight">New Appointment</DialogTitle>
                    <DialogDescription className="text-xl font-medium text-muted-foreground/80">Reserve your expert medical consultation.</DialogDescription>
                  </DialogHeader>
                        ...
                        <SelectContent className="rounded-3xl border-none shadow-2xl p-2">
                          {doctors.map(d => (
                            <SelectItem key={d._id} value={d._id} className="h-14 text-lg rounded-2xl">
                              Dr. {d.name} — <span className="text-primary font-bold">{d.specialization}</span>
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="outline" className="w-full h-16 rounded-2xl border-muted/20 bg-muted/5 justify-start text-left font-bold text-lg">
                              ...
                              {newBooking.appointmentDate ? newBooking.appointmentDate.toLocaleDateString() : <span>Pick a date</span>}
                            </Button>
                          </PopoverTrigger>
                  ...
                  <div className="flex gap-6 pt-8">
                    <Button variant="ghost" onClick={() => setBookingDialogOpen(false)} className="rounded-full h-16 flex-1 font-black text-lg">Cancel</Button>
                    <Button onClick={handleBook} className="rounded-full bg-primary h-16 flex-1 font-black text-xl shadow-2xl">Confirm</Button>
                  </div>
                </div>
              </div>
            </DialogContent>
                        ...
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <div className="font-black text-xl text-primary">
                              {isDoctor ? 'Patient Record' : `Dr. ${doctors.find(d => d._id === a.doctorId)?.name || 'Specialist'}`}
                            </div>
                      ...
                      <td className="py-8 px-10">
                        <div className="space-y-1">
                          <div className="font-black text-lg flex items-center gap-2">
                            <CalendarIcon className="w-5 h-5 text-primary/60" /> {new Date(a.appointmentDate).toLocaleDateString()}
                          </div>
                      ...
                      <td className="py-8 px-10 text-right">
                        <div className="flex items-center justify-end gap-3">
                          {isDoctor && a.status === 'pending' && (
                            <>
                              <Button size="icon" className="rounded-full bg-green-500 hover:bg-green-600" onClick={() => handleStatusUpdate(a._id, 'confirmed')}><Check className="w-5 h-5" /></Button>
                              <Button size="icon" variant="destructive" className="rounded-full" onClick={() => handleStatusUpdate(a._id, 'cancelled')}><X className="w-5 h-5" /></Button>
                            </>
                          )}
                          {a.status === 'confirmed' && (
                          ...
                          {isDoctor && a.status === 'confirmed' && (
                            <Button variant="outline" className="rounded-full h-10 px-6 font-bold" onClick={() => handleStatusUpdate(a._id, 'completed')}>Complete</Button>
                          )}
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="rounded-2xl border-none shadow-xl p-2 min-w-[200px]">
                              <DropdownMenuItem className="rounded-xl h-12 text-lg font-bold cursor-pointer" onClick={() => { setSelectedAppointment(a); setDetailsDialogOpen(true); }}>Details</DropdownMenuItem>
                              {!isDoctor && (a.status === 'pending' || a.status === 'confirmed') && (
                                <DropdownMenuItem className="rounded-xl h-12 text-lg font-bold text-red-500 cursor-pointer" onClick={() => handleCancel(a._id)}>Cancel</DropdownMenuItem>
                              )}
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
          ...
          {selectedAppointment && (
            <div className="space-y-6 py-6">
              <div className="bg-muted/20 p-6 rounded-3xl space-y-4">
                <div className="flex justify-between text-sm uppercase tracking-widest font-black text-muted-foreground"><span>Status</span><span className="text-primary">{selectedAppointment.status}</span></div>
                <div className="flex justify-between text-sm uppercase tracking-widest font-black text-muted-foreground"><span>Mode</span><span className="text-primary">{selectedAppointment.consultationType}</span></div>
                <div className="flex justify-between text-sm uppercase tracking-widest font-black text-muted-foreground"><span>Time</span><span className="text-primary">{selectedAppointment.startTime}</span></div>
              </div>
              <div className="space-y-2">
                <p className="text-xs font-black text-muted-foreground uppercase tracking-widest">Reason for Visit</p>
                <p className="font-medium text-lg leading-relaxed italic">"{selectedAppointment.reasonForVisit || 'No description provided'}"</p>
              </div>
            </div>
          )}
